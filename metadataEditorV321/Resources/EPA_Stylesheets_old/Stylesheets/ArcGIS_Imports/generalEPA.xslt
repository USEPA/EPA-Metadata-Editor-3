...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:res="http://www.esri.com/metadata/res/" xmlns:esri="http://www.esri.com/metadata/" xmlns:gmd="http://www.isotc211.org/2005/gmd" >

  <xsl:import href = "codelistsEPA.xslt" />
...
<!-- add target to links if one isn't alreayd present -->
...
<xsl:template name="elementText">
	...
	<xsl:choose>
		...
		<!-- ESRI Locales -->
		<xsl:when test="($eleName = 'language')"><res:idLocale/></xsl:when>
		<!-- Metadata Info -->
		...
		<!-- Services -->
		<xsl:when test="($eleName = 'svType')"><res:svType/></xsl:when>
		...
		<xsl:when test="($eleName = 'genericName')"><res:catFetTyps/></xsl:when>
