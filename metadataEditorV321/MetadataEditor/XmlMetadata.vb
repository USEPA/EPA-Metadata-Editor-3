...
Public Class XmlMetadata
    ...
    Sub New(xml As String, Optional defaultRoot As String = "dummy")
        If xml.Trim = "" Then xml = "<" + defaultRoot + "/>"
        SetXml(xml)
    End Sub
    ...
    Public Sub copyFrom(ByVal src As XmlMetadata, ByVal xpathStrSource As String, Optional ByVal xpathStrTarget As String = Nothing)
        ...
        If src.SimpleGetProperty("/").Trim = "" Then
            ' Simply delete the target if the source to be copied to it does not contain any text
            Me.DeleteProperty(xpathStrTarget)
        Else
            ' Get source xml
            Dim srcXml As String = src.GetXml(xpathStrSource)
            ' Copy to target xpath - only if there is anything to copy.
            'If srcXml IsNot Nothing AndAlso srcXml.Trim <> "" Then 
            If srcXml.Trim <> "" Then Me.SetXml(xpathStrTarget, srcXml)
        End If
    End Sub
    ...
    ''' <remarks></remarks>
    Public Sub copyFromString(ByVal srcXml As String, ByVal xpathStrTarget As String)
        ' Copy to target xpath - only if there is anything to copy.
    ...
    Public Sub checkDeleteKTTags()
        ...
        End If
        If SimpleGetProperty("idinfo/keywords") = "" Then deletePropertyAndEmptyParents("idinfo/keywords")
    End Sub
    ...
    ''' <remarks></remarks>
    Public Sub deletePropertyAndEmptyParents(ByVal name As String)
        Do
            DeleteProperty(name)
            name = getTagParent(name)
        Loop While name <> "" AndAlso tagIsEmpty(name)
    End Sub
    ...
    Public Function tagIsEmpty(ByVal name As String) As Boolean
        Return Utils.nv(SimpleGetProperty(name)).ToString().Trim() = ""
    End Function
