...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="errs">
		<table>
			<thead>
				<tr>
					<th colspan="2">EPA Metadata Editor - Validation Results - <xsl:value-of select="//ObjectName" /></th>
				</tr>
			</thead>
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<tr>
						<th>Summary</th>
						<td>
							<xsl:value-of select="count(err[type='Error'])"/> error(s),
							<xsl:value-of select="count(err[type='Warning'])"/> warning(s).
						</td>
					</tr>
				</xsl:otherwise>
			</xsl:choose>
